...
#include <span.hpp>
#elif __has_include("tcb/span.hpp")
...
namespace gpufmt {
    ...
    gpufmt::span<const gpufmt::byte> asBytes(const T &value) {
        return gpufmt::span<const gpufmt::byte>{reinterpret_cast<const gpufmt::byte*>(&value), sizeof(T)};
    }

    template<class T>
    gpufmt::span<gpufmt::byte> asWritableBytes(T &value) {
        return gpufmt::span<gpufmt::byte>{reinterpret_cast<gpufmt::byte*>(&value), sizeof(T)};
    }
